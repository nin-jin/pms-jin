 ...
 *
 * Typical usage:
 *     var formatTime = $jin.time1.format( 'Weekday, YYYY-MM-DD hh:mm' )
 *     formatTime( $jin.time1.moment() )
 *
 * @name $jin.time1.format
 * @method format
 * @member $jin.time
 ...
 * @static
 */
$jin.property.hash({ '$jin.time1.format': { pull: function( pattern ) {

	var lexems = $jin.time1.format()
	...
	pattern.replace( lexer, function( str, text, token ){
		if( text ) funcs.push( function(){ return text } )
