module $jin.prop.test {
    ...
    $jin.test( test => {
        var atom1 = new $jin.atom.prop<number>({
            pull : prev => 0
        })
        var atom2 = new $jin.atom.prop<number>({
            pull : prev => atom1.get() + 111
        })
        var atom3 = new $jin.atom.prop<number>({
    ...
    class AtomTriplet extends $jin.object {
        get value1( ) {
            return new $jin.atom.prop<number>( {
                owner : this,
                name : '_value1'
            } )
        }
        ...
        get value3( ) {
            return new $jin.atom.prop<number>( {
                owner : this,
                name : '_value3',
                pull : prev => this.value1.get() && this.value2.get() 
            } )
        }
    }
